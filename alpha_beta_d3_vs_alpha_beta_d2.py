...
def describe_agent(agent):
    ...
    if isinstance(agent, AlphaBetaAgent):
        ...
        return f"{cls}_Depth{agent.max_depth}"
...
def run_matchup(name, agentA_proto, agentB_proto, games, starts_for_A, board_size=15):
    ...
    for g in range(games):
        ...
        if g < starts_for_A:
            ...
            cut1, cut2 = stats["cutoffs_1"], stats["cutoffs_2"]
            ...
            cut2, cut1 = stats["cutoffs_1"], stats["cutoffs_2"]
            mt2, mt1 = stats["move_times_1"], stats["move_times_2"]

        if winner_sign == 1:
            winner = starter
            if starter == agent1_label: wins1 += 1
            else: wins2 += 1
        elif winner_sign == -1:
            winner = agent2_label if starter == agent1_label else agent1_label
            if winner == agent1_label: wins1 += 1
            else: wins2 += 1
        else:
            winner = "Draw"
        ...
        e1, m1, l1, o1 = compute_phase_averages(mt1, moves)
        e2, m2, l2, o2 = compute_phase_averages(mt2, moves)
        ...
        records.append(rec)
        print(f"  -> Game {g+1}/{games} complete")
...
def run_all_experiments():
    ...
    records = run_matchup(
        name="AB_d2_vs_AB_d3",
        agentA_proto=AlphaBetaAgent(depth=2, proximity=2),
        agentB_proto=AlphaBetaAgent(depth=3, proximity=2),
        games=8,
        starts_for_A=4,
        board_size=15
    )

    export_csv("gomoku_AB_d2_vs_AB_d3_1.csv", records)
